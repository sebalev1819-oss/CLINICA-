...
create table public.pacientes (
  id              uuid primary key default uuid_generate_v4(),
  ref             text unique not null,         -- PAC-2025-XXXX
  nombre          text not null,
  dni             text not null,
  telefono        text,
  email           text,
...
-- Constraint anti-solapamiento: mismo consultorio no puede tener dos turnos activos al mismo tiempo
-- Se crea separado para poder usar btree_gist correctamente
-- Nota: duracion_min::text es necesario porque PostgreSQL no concatena int + text directamente
alter table public.turnos
  ...
  exclude using gist (
    ...
    tsrange(
      ...
      (fecha + hora + (duracion_min::text || ' minutes')::interval)::timestamp
    ) with &&
...
create table public.autorizaciones (
  ...
  sesiones_auth     int not null default 0,
